...
learning_rate = 0.1 # 0.001, 0.1 <- magic values
reg_param = 0
max_iter = 5000
m = 4 # Number of training examples

# The model needs to be over fit to make predictio
np.random.seed(1)
W1 = np.random.normal(0, 1, (num_h_units, num_i_units)) # 2 X 2
W2 = np.random.normal(0, 1, (num_o_units, num_h_units)) # 1 X 2

B1 = np.random.random((num_h_units, 1)) # 2 X 1
B2 = np.random.random((num_o_units, 1)) # 1 X 1
